...
/*******************************************************************************
* Function Name: ADV_VPOT_theACLK_Start
********************************************************************************
*
* Summary:
...
void ADV_VPOT_theACLK_Start(void) 
{
    ...
    ADV_VPOT_theACLK_CLKEN |= ADV_VPOT_theACLK_CLKEN_MASK;
	ADV_VPOT_theACLK_CLKSTBY |= ADV_VPOT_theACLK_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: ADV_VPOT_theACLK_Stop
********************************************************************************
*
* Summary:
...
void ADV_VPOT_theACLK_Stop(void) 
{
    ...
    ADV_VPOT_theACLK_CLKEN &= (uint8)(~ADV_VPOT_theACLK_CLKEN_MASK);
	ADV_VPOT_theACLK_CLKSTBY &= (uint8)(~ADV_VPOT_theACLK_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: ADV_VPOT_theACLK_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((ADV_VPOT_theACLK_CLKEN & ADV_VPOT_theACLK_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = ADV_VPOT_theACLK_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = ADV_VPOT_theACLK_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* ADV_VPOT_theACLK__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(ADV_VPOT_theACLK_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(ADV_VPOT_theACLK_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: ADV_VPOT_theACLK_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: ADV_VPOT_theACLK_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = ADV_VPOT_theACLK_GetSourceRegister();
    uint16 oldDivider = ADV_VPOT_theACLK_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = ADV_VPOT_theACLK_CLKEN & ADV_VPOT_theACLK_CLKEN_MASK;
        ...
        if ((currSrc == (uint8)CYCLK_SRC_SEL_CLK_SYNC_D) && ((oldDivider == 0u) || (clkDivider == 0u)))
        {
            ...
            if (oldDivider == 0u)
            {
                ...
                CY_SET_REG16(ADV_VPOT_theACLK_DIV_PTR, clkDivider);
                ADV_VPOT_theACLK_MOD_SRC &= (uint8)(~CYCLK_SSS);
            }
            else
            {
                /* Moving to SSS, set SSS which then ignores the divider and we can set     */
                /* it without bothering with the shadow load.                               */
                ADV_VPOT_theACLK_MOD_SRC |= CYCLK_SSS;
                CY_SET_REG16(ADV_VPOT_theACLK_DIV_PTR, clkDivider);
            }
        }
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((ADV_VPOT_theACLK_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
            ...
            else
            {
                ...
                CY_SET_REG16(ADV_VPOT_theACLK_DIV_PTR, clkDivider);
				ADV_VPOT_theACLK_CLKEN |= enabled;
            }
        }
    }
}
...
/*******************************************************************************
* Function Name: ADV_VPOT_theACLK_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(ADV_VPOT_theACLK_DIV_PTR);
}


/*******************************************************************************
* Function Name: ADV_VPOT_theACLK_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    ADV_VPOT_theACLK_MOD_SRC |= modeBitMask & (uint8)ADV_VPOT_theACLK_MODE_MASK;
}


/*******************************************************************************
* Function Name: ADV_VPOT_theACLK_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    ADV_VPOT_theACLK_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(ADV_VPOT_theACLK_MODE_MASK));
}


/*******************************************************************************
* Function Name: ADV_VPOT_theACLK_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return ADV_VPOT_theACLK_MOD_SRC & (uint8)(ADV_VPOT_theACLK_MODE_MASK);
}


/*******************************************************************************
* Function Name: ADV_VPOT_theACLK_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = ADV_VPOT_theACLK_GetDividerRegister();
    uint8 oldSrc = ADV_VPOT_theACLK_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        ADV_VPOT_theACLK_MOD_SRC |= CYCLK_SSS;
        ADV_VPOT_theACLK_MOD_SRC =
            (ADV_VPOT_theACLK_MOD_SRC & (uint8)(~ADV_VPOT_theACLK_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: ADV_VPOT_theACLK_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return ADV_VPOT_theACLK_MOD_SRC & ADV_VPOT_theACLK_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: ADV_VPOT_theACLK_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    ADV_VPOT_theACLK_PHASE = clkPhase & ADV_VPOT_theACLK_PHASE_MASK;
}


/*******************************************************************************
* Function Name: ADV_VPOT_theACLK_GetPhase
********************************************************************************
*
* Summary:
...
{
    return ADV_VPOT_theACLK_PHASE & ADV_VPOT_theACLK_PHASE_MASK;
}
