...
/*******************************************************************************
* Function Name: SPI_FNG_CLK_Start
********************************************************************************
*
* Summary:
...
void SPI_FNG_CLK_Start(void) 
{
    ...
    SPI_FNG_CLK_CLKEN |= SPI_FNG_CLK_CLKEN_MASK;
	SPI_FNG_CLK_CLKSTBY |= SPI_FNG_CLK_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: SPI_FNG_CLK_Stop
********************************************************************************
*
* Summary:
...
void SPI_FNG_CLK_Stop(void) 
{
    ...
    SPI_FNG_CLK_CLKEN &= (uint8)(~SPI_FNG_CLK_CLKEN_MASK);
	SPI_FNG_CLK_CLKSTBY &= (uint8)(~SPI_FNG_CLK_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: SPI_FNG_CLK_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((SPI_FNG_CLK_CLKEN & SPI_FNG_CLK_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = SPI_FNG_CLK_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = SPI_FNG_CLK_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* SPI_FNG_CLK__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(SPI_FNG_CLK_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(SPI_FNG_CLK_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: SPI_FNG_CLK_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: SPI_FNG_CLK_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = SPI_FNG_CLK_GetSourceRegister();
    uint16 oldDivider = SPI_FNG_CLK_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = SPI_FNG_CLK_CLKEN & SPI_FNG_CLK_CLKEN_MASK;
        ...
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((SPI_FNG_CLK_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
...
/*******************************************************************************
* Function Name: SPI_FNG_CLK_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(SPI_FNG_CLK_DIV_PTR);
}


/*******************************************************************************
* Function Name: SPI_FNG_CLK_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    SPI_FNG_CLK_MOD_SRC |= modeBitMask & (uint8)SPI_FNG_CLK_MODE_MASK;
}


/*******************************************************************************
* Function Name: SPI_FNG_CLK_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    SPI_FNG_CLK_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(SPI_FNG_CLK_MODE_MASK));
}


/*******************************************************************************
* Function Name: SPI_FNG_CLK_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return SPI_FNG_CLK_MOD_SRC & (uint8)(SPI_FNG_CLK_MODE_MASK);
}


/*******************************************************************************
* Function Name: SPI_FNG_CLK_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = SPI_FNG_CLK_GetDividerRegister();
    uint8 oldSrc = SPI_FNG_CLK_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        SPI_FNG_CLK_MOD_SRC |= CYCLK_SSS;
        SPI_FNG_CLK_MOD_SRC =
            (SPI_FNG_CLK_MOD_SRC & (uint8)(~SPI_FNG_CLK_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: SPI_FNG_CLK_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return SPI_FNG_CLK_MOD_SRC & SPI_FNG_CLK_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: SPI_FNG_CLK_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    SPI_FNG_CLK_PHASE = clkPhase & SPI_FNG_CLK_PHASE_MASK;
}


/*******************************************************************************
* Function Name: SPI_FNG_CLK_GetPhase
********************************************************************************
*
* Summary:
...
{
    return SPI_FNG_CLK_PHASE & SPI_FNG_CLK_PHASE_MASK;
}
