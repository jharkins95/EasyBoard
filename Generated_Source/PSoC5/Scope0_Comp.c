...
uint8 Scope0_Comp_initVar = 0u;

/* Internal functions definitoin */
static void Scope0_Comp_trimAdjust(uint8 nibble) ;

/* static Scope0_Comp_backupStruct  Scope0_Comp_backup; */
#if (CY_PSOC5A)
    static Scope0_Comp_LOWPOWER_BACKUP_STRUCT  Scope0_Comp_lowPowerBackup;
#endif /* CY_PSOC5A */

/* variable to decide whether or not to restore the control register in 
   Enable() API for PSoC5A only */
#if (CY_PSOC5A)
    static uint8 Scope0_Comp_restoreReg = 0u;
#endif /* CY_PSOC5A */


/*******************************************************************************
* Function Name: Scope0_Comp_Init
********************************************************************************
*
* Summary:
...
void Scope0_Comp_Init(void) 
{
    ...
    Scope0_Comp_SetSpeed(Scope0_Comp_DEFAULT_SPEED);

    /* Set default Hysteresis */
    #if ( Scope0_Comp_DEFAULT_HYSTERESIS == 0u )
        Scope0_Comp_CR |= Scope0_Comp_HYST_OFF;
    #else
        Scope0_Comp_CR &= (uint8)(~Scope0_Comp_HYST_OFF);
    #endif /* Scope0_Comp_DEFAULT_HYSTERESIS == 0u */
    /* Power down override feature is not supported for PSoC5A. */
    #if (CY_PSOC3 || CY_PSOC5LP)
        /* Set default Power Down Override */
        #if ( Scope0_Comp_DEFAULT_PWRDWN_OVRD == 0u )
            Scope0_Comp_CR &= (uint8)(~Scope0_Comp_PWRDWN_OVRD);
        #else 
            Scope0_Comp_CR |= Scope0_Comp_PWRDWN_OVRD;
        #endif /* Scope0_Comp_DEFAULT_PWRDWN_OVRD == 0u */
    #endif /* CY_PSOC3 || CY_PSOC5LP */
    
    /* Set mux always on logic */
    Scope0_Comp_CR |= Scope0_Comp_MX_AO;

    /* Set default sync */
    Scope0_Comp_CLK &= (uint8)(~Scope0_Comp_SYNCCLK_MASK);
    #if ( Scope0_Comp_DEFAULT_BYPASS_SYNC == 0u )
        Scope0_Comp_CLK |= Scope0_Comp_SYNC_CLK_EN;
    #else
        Scope0_Comp_CLK |= Scope0_Comp_BYPASS_SYNC;
    #endif /* Scope0_Comp_DEFAULT_BYPASS_SYNC == 0u */
}


/*******************************************************************************
* Function Name: Scope0_Comp_Enable
********************************************************************************
*
* Summary:
...
{
    Scope0_Comp_PWRMGR |= Scope0_Comp_ACT_PWR_EN;
    Scope0_Comp_STBY_PWRMGR |= Scope0_Comp_STBY_PWR_EN;
     
     /* This is to restore the value of register CR which is saved 
    in prior to the modification in stop() API */
    #if (CY_PSOC5A)
        if(Scope0_Comp_restoreReg == 1u)
        {
            Scope0_Comp_CR = Scope0_Comp_lowPowerBackup.compCRReg;

            /* Clear the flag */
            Scope0_Comp_restoreReg = 0u;
        }
    #endif /* CY_PSOC5A */
}
...
/*******************************************************************************
* Function Name: Scope0_Comp_Start
********************************************************************************
*
* Summary:
...
*
* Global variables:
*  Scope0_Comp_initVar: Is modified when this function is called for the 
...
void Scope0_Comp_Start(void) 
{
    ...
    if ( Scope0_Comp_initVar == 0u )
    {
        Scope0_Comp_Init();
        
        Scope0_Comp_initVar = 1u;
    }   
...
/*******************************************************************************
* Function Name: Scope0_Comp_Stop
********************************************************************************
*
* Summary:
...
void Scope0_Comp_Stop(void) 
{
    ...
    Scope0_Comp_PWRMGR &= (uint8)(~Scope0_Comp_ACT_PWR_EN);
    Scope0_Comp_STBY_PWRMGR &= (uint8)(~Scope0_Comp_STBY_PWR_EN);
    
    #if (CY_PSOC5A)
        /* Enable the variable */
        Scope0_Comp_restoreReg = 1u;

        /* Save the control register before clearing it */
        Scope0_Comp_lowPowerBackup.compCRReg = Scope0_Comp_CR;
        Scope0_Comp_CR = Scope0_Comp_COMP_REG_CLR;
    #endif /* CY_PSOC5A */
}


/*******************************************************************************
* Function Name: Scope0_Comp_SetSpeed
********************************************************************************
*
* Summary:
...
void Scope0_Comp_SetSpeed(uint8 speed) 
{
    ...
    Scope0_Comp_CR = (Scope0_Comp_CR & (uint8)(~Scope0_Comp_PWR_MODE_MASK)) |
                           (speed & Scope0_Comp_PWR_MODE_MASK);

    /* Set trim value for high speed comparator */
    if(speed == Scope0_Comp_HIGHSPEED)
    {
        /* PSoC5A */
        #if (CY_PSOC5A)
            Scope0_Comp_TR = Scope0_Comp_HS_TRIM_TR0;
        #endif /* CY_PSOC5A */
        
        /* PSoC3, PSoC5LP or later */
        #if (CY_PSOC3 || CY_PSOC5LP) 
            Scope0_Comp_TR0 = Scope0_Comp_HS_TRIM_TR0;
            Scope0_Comp_TR1 = Scope0_Comp_HS_TRIM_TR1;
        #endif /* CY_PSOC3 || CY_PSOC5LP */
    }
    else
    {
    /* PSoC5A */
        #if (CY_PSOC5A)
            Scope0_Comp_TR = Scope0_Comp_LS_TRIM_TR0;
        #endif /* CY_PSOC5A */
        
        /* PSoC3, PSoC5LP or later */
        #if (CY_PSOC3 || CY_PSOC5LP) 
            Scope0_Comp_TR0 = Scope0_Comp_LS_TRIM_TR0;
            Scope0_Comp_TR1 = Scope0_Comp_LS_TRIM_TR1;
        #endif /* CY_PSOC3 || CY_PSOC5LP */
    }
...
/*******************************************************************************
* Function Name: Scope0_Comp_GetCompare
********************************************************************************
*
* Summary:
...
{
    return( Scope0_Comp_WRK & Scope0_Comp_CMP_OUT_MASK);
}


/*******************************************************************************
* Function Name: Scope0_Comp_trimAdjust
********************************************************************************
*
* Summary:
...
static void Scope0_Comp_trimAdjust(uint8 nibble) 
{
    ...
    cmpState = Scope0_Comp_WRK & Scope0_Comp_CMP_OUT_MASK;
    ...
    {    
        ...
        if ( cmpState != 0u )
        {
            /* PSoC5A */
            #if (CY_PSOC5A)
                Scope0_Comp_TR |= Scope0_Comp_CMP_TRIM1_DIR;
            #endif /* CY_PSOC5A */
            
            /* PSoC3, PSoC5LP or later */
            #if (CY_PSOC3 || CY_PSOC5LP)
                Scope0_Comp_TR0 |= Scope0_Comp_CMP_TR0_DIR;
            #endif /* CY_PSOC3 || CY_PSOC5LP */
        }
    }
    else
    {
        ...
        if ( cmpState == 0u )
        {
            /* PSoC5A */
            #if (CY_PSOC5A)
                Scope0_Comp_TR |= Scope0_Comp_CMP_TRIM2_DIR; 
            #endif /* CY_PSOC5A */
            
            /* PSoC3, PSoC5LP or later */
            #if (CY_PSOC3 || CY_PSOC5LP)
                Scope0_Comp_TR1 |= Scope0_Comp_CMP_TR1_DIR;
            #endif /* CY_PSOC3 || CY_PSOC5LP */
        }
    }
    ...
    for ( trimCnt = 0u; trimCnt < trimCntMax; trimCnt++ )
	{
        ...
        if ((Scope0_Comp_WRK & Scope0_Comp_CMP_OUT_MASK) != cmpState)
        {
...
/*******************************************************************************
* Function Name: Scope0_Comp_ZeroCal
********************************************************************************
*
* Summary:
...
uint16 Scope0_Comp_ZeroCal(void) 
{
    ...
    tmpSW0 = Scope0_Comp_SW0;
    tmpSW2 = Scope0_Comp_SW2;
    tmpSW3 = Scope0_Comp_SW3;

     /* Clear routing for inP, retain routing for inN */
    Scope0_Comp_SW0 = 0x00u;
    Scope0_Comp_SW2 = 0x00u;
    Scope0_Comp_SW3 = tmpSW3 & (uint8)(~Scope0_Comp_CMP_SW3_INPCTL_MASK);

    /* Preserve original configuration
     * - turn off Hysteresis
     * - set calibration bit - shorts inN to inP
    */
    tmpCR = Scope0_Comp_CR;
    Scope0_Comp_CR |= (Scope0_Comp_CAL_ON | Scope0_Comp_HYST_OFF);
    
    /* Write default low values to trim register - no offset adjust */
    /* PSoC5A */
    #if (CY_PSOC5A)
        Scope0_Comp_TR = Scope0_Comp_DEFAULT_CMP_TRIM;
    #endif /* CY_PSOC5A */
    
    /* PSoC3, PSoC5LP or later */
    #if (CY_PSOC3 || CY_PSOC5LP)
        Scope0_Comp_TR0 = Scope0_Comp_DEFAULT_CMP_TRIM;
        Scope0_Comp_TR1 = Scope0_Comp_DEFAULT_CMP_TRIM;
    #endif /* CY_PSOC3 || CY_PSOC5LP */
	
	/* Two phase trim - slow modes, one phase trim - for fast */ 
    if ( (Scope0_Comp_CR & Scope0_Comp_PWR_MODE_MASK) == Scope0_Comp_PWR_MODE_FAST)
    {
        Scope0_Comp_trimAdjust(0u);
    }
    else /* default to trim for fast modes */
    {
        Scope0_Comp_trimAdjust(1u);
		Scope0_Comp_trimAdjust(0u);
    }
   
    /* Restore Config Register */
    Scope0_Comp_CR = tmpCR;
    
    /* Restore routing registers for inP */
    Scope0_Comp_SW0 = tmpSW0;
    Scope0_Comp_SW2 = tmpSW2;
    Scope0_Comp_SW3 = tmpSW3;
    
    /* PSoC5A */
    #if (CY_PSOC5A)
        return (uint16) Scope0_Comp_TR;
    #endif /* CY_PSOC5A */
    
    /* PSoC3, PSoC5LP or later */
    #if (CY_PSOC3 || CY_PSOC5LP)
        return ((uint16)((uint16)Scope0_Comp_TR1 << 8u) | (Scope0_Comp_TR0));        
    #endif /* CY_PSOC3 || CY_PSOC5LP */
}


/*******************************************************************************
* Function Name: Scope0_Comp_LoadTrim
********************************************************************************
*
* Summary:
...
void Scope0_Comp_LoadTrim(uint16 trimVal) 
{
    ...
    /*******************************************************************************
    * Function Name: Scope0_Comp_PwrDwnOverrideEnable
    ********************************************************************************
    *
    * Summary:
    ...
    *******************************************************************************/
    ...
    /*******************************************************************************
    * Function Name: Scope0_Comp_PwrDwnOverrideDisable
    ********************************************************************************
    *
    * Summary:
