...
/*******************************************************************************
* Function Name: WaveDAC_Low_Clock_Start
********************************************************************************
*
* Summary:
...
void WaveDAC_Low_Clock_Start(void) 
{
    ...
    WaveDAC_Low_Clock_CLKEN |= WaveDAC_Low_Clock_CLKEN_MASK;
	WaveDAC_Low_Clock_CLKSTBY |= WaveDAC_Low_Clock_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: WaveDAC_Low_Clock_Stop
********************************************************************************
*
* Summary:
...
void WaveDAC_Low_Clock_Stop(void) 
{
    ...
    WaveDAC_Low_Clock_CLKEN &= (uint8)(~WaveDAC_Low_Clock_CLKEN_MASK);
	WaveDAC_Low_Clock_CLKSTBY &= (uint8)(~WaveDAC_Low_Clock_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: WaveDAC_Low_Clock_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((WaveDAC_Low_Clock_CLKEN & WaveDAC_Low_Clock_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = WaveDAC_Low_Clock_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = WaveDAC_Low_Clock_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* WaveDAC_Low_Clock__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(WaveDAC_Low_Clock_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(WaveDAC_Low_Clock_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: WaveDAC_Low_Clock_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: WaveDAC_Low_Clock_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = WaveDAC_Low_Clock_GetSourceRegister();
    uint16 oldDivider = WaveDAC_Low_Clock_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = WaveDAC_Low_Clock_CLKEN & WaveDAC_Low_Clock_CLKEN_MASK;
        ...
        if ((currSrc == (uint8)CYCLK_SRC_SEL_CLK_SYNC_D) && ((oldDivider == 0u) || (clkDivider == 0u)))
        {
            ...
            if (oldDivider == 0u)
            {
                ...
                CY_SET_REG16(WaveDAC_Low_Clock_DIV_PTR, clkDivider);
                WaveDAC_Low_Clock_MOD_SRC &= (uint8)(~CYCLK_SSS);
            }
            else
            {
                /* Moving to SSS, set SSS which then ignores the divider and we can set     */
                /* it without bothering with the shadow load.                               */
                WaveDAC_Low_Clock_MOD_SRC |= CYCLK_SSS;
                CY_SET_REG16(WaveDAC_Low_Clock_DIV_PTR, clkDivider);
            }
        }
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((WaveDAC_Low_Clock_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
            ...
            else
            {
                ...
                CY_SET_REG16(WaveDAC_Low_Clock_DIV_PTR, clkDivider);
				WaveDAC_Low_Clock_CLKEN |= enabled;
            }
        }
    }
}
...
/*******************************************************************************
* Function Name: WaveDAC_Low_Clock_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(WaveDAC_Low_Clock_DIV_PTR);
}


/*******************************************************************************
* Function Name: WaveDAC_Low_Clock_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    WaveDAC_Low_Clock_MOD_SRC |= modeBitMask & (uint8)WaveDAC_Low_Clock_MODE_MASK;
}


/*******************************************************************************
* Function Name: WaveDAC_Low_Clock_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    WaveDAC_Low_Clock_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(WaveDAC_Low_Clock_MODE_MASK));
}


/*******************************************************************************
* Function Name: WaveDAC_Low_Clock_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return WaveDAC_Low_Clock_MOD_SRC & (uint8)(WaveDAC_Low_Clock_MODE_MASK);
}


/*******************************************************************************
* Function Name: WaveDAC_Low_Clock_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = WaveDAC_Low_Clock_GetDividerRegister();
    uint8 oldSrc = WaveDAC_Low_Clock_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        WaveDAC_Low_Clock_MOD_SRC |= CYCLK_SSS;
        WaveDAC_Low_Clock_MOD_SRC =
            (WaveDAC_Low_Clock_MOD_SRC & (uint8)(~WaveDAC_Low_Clock_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: WaveDAC_Low_Clock_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return WaveDAC_Low_Clock_MOD_SRC & WaveDAC_Low_Clock_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: WaveDAC_Low_Clock_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    WaveDAC_Low_Clock_PHASE = clkPhase & WaveDAC_Low_Clock_PHASE_MASK;
}


/*******************************************************************************
* Function Name: WaveDAC_Low_Clock_GetPhase
********************************************************************************
*
* Summary:
...
{
    return WaveDAC_Low_Clock_PHASE & WaveDAC_Low_Clock_PHASE_MASK;
}
